...
if [ "${TRAVIS_PULL_REQUEST}" = "false" ]; then
	case "${TRAVIS_BRANCH}" in
		"next")
			...
			./deploy.sh --env dev --state-bucket auscors-terraform-state-dev
		;;
		"master")
			aws configure set aws_access_key_id "${TRAVIS_AWS_ACCESS_KEY_ID}"
			aws configure set aws_secret_access_key "${TRAVIS_AWS_SECRET_KEY_ID}"
			aws configure set region ap-southeast-2
			./deploy.sh --env test --state-bucket auscors-terraform-state-dev
		;;
		"prod")
			...
			aws configure set region ap-southeast-2
			./deploy.sh --env prod --state-bucket auscors-terraform-state-prod
		;;
	esac
fi
